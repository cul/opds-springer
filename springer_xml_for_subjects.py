...
from springer.model import SessionManager, Identifier, Edition, Subject, Classification, Contribution, DataSource, get_one_or_create, NONFICTION_CLASSIFIER
...
def opds_publication(identifier, timestamp):
    ...
    for resource in identifier.resources:
        ...
        resources.append({
            ...
            'href': "https://fsso.springer.com/saml/login?idp=urn:mace:incommon:columbia.edu&targetUrl=%s" % (resourceUrl)
        })
...
def opds_response_link(page_number, rel):
    return {
        "rel": rel,
        "href": ("https://ebooks-test.library.columbia.edu/static-feeds/springer/springer_test_feed_%s.xml" % page_number),
...
def cache_page(output_base_dir, opds_page, page_number):
    out_file = os.path.join(output_base_dir, ("springer_test_feed_%s.xml" % page_number))
    with open(os.path.join(out_file), "w") as f:
        open_xml(out_file)
        write_link_elements(opds_page, out_file)
        close_xml(out_file)
        json.dump(opds_page, f, indent=2)

def main():
    output_base_dir = "output_test/springer/opds-xml"
    ...
    load_query = load_query.limit(page_size - 1)
    offset = 0
    while offset == 0:
